...
async function fetchMentorClientDashboard(clientId) {
  ...
  const latestSession = sessions.sessions && sessions.sessions.length ? sessions.sessions[0] : null;

  if (!latestSession) {
    return { client, sessions: [], history: null };
  }

  const history = await apiRequest(`/api/sessions/${latestSession.id}/history`, { method: "GET" });
  return { client, sessions: sessions.sessions || [], history };
}
...
function renderMentorPanel() {
  ...
  const activateClient = async clientId => {
    ...
    try {
      const dashboard = await fetchMentorClientDashboard(clientId);
  ...
  void (async () => {
    try {
      ...
      if (currentClientId) {
        await activateClient(currentClientId);
...
document.getElementById("pdfBtn").onclick = () => {
  (async () => {
    try {
      const sessionId = currentSessionId();
      if (sessionId) {
        await apiRequest(`/api/sessions/${sessionId}/reports`, {
          method: "POST",
          body: JSON.stringify({ report_path: "browser-print" }),
        });
      }
    } catch (error) {
  ...
  let report = `
    <html>
    <head>
      <title>Relatório Estratégico Casa Clube</title>
      <style>
        body { font-family: Georgia, serif; padding: 40px; color: #2f2f2f; line-height: 1.6; }
        h1, h2 { color: #6d302d; }
        h1 { margin-bottom: 8px; }
        .meta { color: #6b7280; font-family: Arial, sans-serif; margin-bottom: 28px; }
        .client-info { margin: 18px 0 28px; padding: 18px; border: 1px solid #e8dccc; border-radius: 14px; background: #fffaf7; }
        .client-info strong { display: block; margin-bottom: 6px; color: #4a1f1d; }
        .answer { white-space: pre-wrap; border-bottom: 1px solid #ddd; padding: 10px 0 18px; margin-bottom: 12px; }
      </style>
    </head>
    <body>
      <h1>Relatório Estratégico Casa Clube®</h1>
      <p class="meta">Gerado a partir do Mapa Estratégico Casa Clube®.</p>
      <div class="client-info">
        <strong>Cliente:</strong> ${escapeHtml(profile.name || "—")}<br>
        <strong>Empresa:</strong> ${escapeHtml(profile.company || "—")}
      </div>
  `;

  steps.forEach(step => {
    report += `<h2>${escapeHtml(step.title)}</h2>`;
    step.fields.forEach(([type, label]) => {
      const value = load(slug(label)) || "—";
      report += `<strong>${escapeHtml(label)}</strong><div class="answer">${escapeHtml(value)}</div>`;
    });

    const note = load(slug("observacao_mentora_" + step.title)) || "—";
    report += `<strong>Observação da mentora</strong><div class="answer">${escapeHtml(note)}</div>`;
  });
